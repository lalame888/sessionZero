...
3. **AI 隊友席**：可新建，或帶入檔案庫角色（同樣佔用、一角同時僅一場）。遊玩中檢定成功會標記該隊友的成長技能；**結局比照玩家做成長結算**，再選寫回／存入檔案庫（含履歷）或略過寫入。
...
* **喚起隊友** (`request_companion_action`)：GM 請 AI 隊友以桌邊玩家口吻宣告（或靜默 pass）。預設軟停等玩家插話／「讓 GM 結算」；危機出手可 `prefer_immediate`／隊友選 `handoff=immediate` 立刻結算。GM **禁止**第三人稱重述隊友台詞；結算檢定／改數值必須帶該隊友 `character_id`。
...
1. 觸發結局，先展示結局敘事與成就。
2. **角色結算與存檔**（一次性）：
   - CoC 若有標記技能 →「執行成長檢定」後**自動**寫入檔案庫履歷並解除綁定。
   ...
   - **AI 隊友**：比照玩家先做成長檢定（CoC）／經驗摘要（D&D）；每位再選「寫回／存入檔案庫（含履歷）」或「略過寫入」。結算玩家時會先解除占用。
...
**雙向綁定**：開始冒險時自動將角色卡存入檔案庫並寫入 `activeCampaignId`；Session 寫入 `boundCharacterId`。刪除 Session 會解除綁定，但已結算的壓縮履歷仍留在角色卡。
